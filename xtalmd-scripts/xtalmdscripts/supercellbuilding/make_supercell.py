...
def main():
    ...
    for f in os.listdir('./CIF'):
        ...
        args.input = str('./CIF/' + f)
        try:
            # args.input = "1100992.cif"
            ...
            with open(f"PDB_supercell/{args.input.split('.')[1].split('/')[2]}_supercell.pdb", "w") as fopen:
                fopen.write(pdb_str)
            with open(f"csv/{args.input.split('.')[1].split('/')[2]}.csv", "w") as fopen:
            ...
            with open(f"json/{args.input.split('.')[1].split('/')[2]}.json", "w") as fopen:
