...
def get_rmsd_r(pdb_path_1, pdb_path_2):
   ...
   # Remove periodicity
    ...
    for atom in range(int()):
        for i in range(nmol):
            for j in range(number_of_atom):
                pos2atomj = pos2_moleculeList_rmv[atom][j]
                pos2ij = pos2ij
                pos1ij = pos1_moleculeList_rmv[i][j]
                for k in range(3):
                    if (abs((pos2atomj[k] - pos2ij[k])) > 0.5 * box_parameter_new[0]):
                        if (pos2atomj[k] < pos2ij[k]):
                            temp_d1 = pos1ij[k] - box_parameter[k]
                            temp_d2 = pos2ij[k] - box_parameter_new[k]
                        if (pos2atomj[k] > pos2ij[k]):
                            temp_d1 = pos1ij[k] + box_parameter[k]
                            temp_d2 = pos2ij[k] + box_parameter_new[k]
                        pos1ij[k] = temp_d1
